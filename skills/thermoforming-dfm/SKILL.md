...
shrinkage; if the mesh already is the tool, do not add it twice.
...
bottom and three-way corners carry a house margin of **3 t** from the depth tables,
where Illig's ladder tops out at 1.5 t — a deliberate margin for working without a plug,
not a law; and the
...
Some conditions make every downstream number meaningless. Check these first and, if one
holds, report it as the finding instead of publishing numbers that look precise:

| Condition | What it invalidates |
|---|---|
| `scale_check.units_suspect` is true | everything — the file is probably in cm or inches |
| post-forming slots, holes and windows still in the model | draft and undercuts: their vertical faces read as zero draft and as undercuts |
| mesh not watertight | undercuts (the field comes back null), and the draped area with it |
| pull direction not settled | draft, undercuts, depth ratio — all are measured along it |
| unclear whether the mesh is the part or the tool | shrinkage applied twice, or not at all |
| blank, clamp and method not stated by the user | every wall number: they set F1 (the tool lists its defaults in `assumptions`) |

**Strip post-forming features** rather than explaining them away: slots, holes and
windows are machined after forming but exist in the model. Measure the shape that will
actually be formed, or subtract those areas explicitly and say so.

A failed gate is reported as a blocker, not as a `❓`: the difference is that a blocker
stops the comparison, while `❓` marks one check that could not be run.
...
4. **Every comparison names both sides of it:** the measured field (`draft.min_deg`,
   `sheet.avg_wall_mm`) and the rule it is compared against — the section of
   `references/rules.md` and, where that section names one, the source behind the number.
   A limit quoted without its origin reads as taste, and the next person cannot tell a
   measured law from a house margin.
5. Layout: parts per blank, with and without a divider bar.
6. **Coverage: what was checked and what was not**, as a list rather than a feeling.
   Draft, undercuts, depth, radii, wall, layout, venting, bubble — each either has a
   result above or appears here with the reason it has none. No findings is not an
   approval; an unlisted check is the one that bites.
7. Assumptions, stated plainly.
8. The three measurement points for the first formed part, with expected values.
